...
fn to_color(num: i16) -> Result<u8, IntoColorError> {
    let color = u8::try_from(num).map_err(|_| IntoColorError::IntConversion)?;
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let red = to_color(tuple.0)?;
        let green = to_color(tuple.1)?;
        let blue = to_color(tuple.2)?;
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let red = to_color(arr[0])?;
        let green = to_color(arr[1])?;
        let blue = to_color(arr[2])?;
        Ok(Color { red, green, blue })
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        let red = to_color(slice[0])?;
        let green = to_color(slice[1])?;
        let blue = to_color(slice[2])?;
        Ok(Color { red, green, blue })
    }
}
